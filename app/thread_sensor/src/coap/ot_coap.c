...
static void publication_work_hanlder(struct k_work *work)
{
	...
	uint8_t msg[PROTO_MSG_MAX_SIZE+1] = {0};
	uint8_t msg_size = proto_msg_size + 1;
	msg[0] = proto_msg_size;
	memcpy(msg+1, proto_buffer, proto_msg_size);

	LOG_WRN("Proto size: %d", proto_msg_size);
	LOG_WRN("Msg size: %d", msg_size);
	...
	memcpy(packet.msg, msg, msg_size);
